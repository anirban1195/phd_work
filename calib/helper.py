...
def correct(sigmaxx_m , sigmayy_m, sigmaxy_m, sigmaxx_p, sigmayy_p, sigmaxy_p, exx_m, eyy_m, exy_m, exx_p, eyy_p, exy_p):
    g1= np.random.normal(0, 1, 30000)
    g2= np.random.normal(0, 1, 30000)
    g3= np.random.normal(0, 1, 30000)
    g4= np.random.normal(0, 1, 30000)
    g5= np.random.normal(0, 1, 30000)
    g6= np.random.normal(0, 1, 30000)
...
def measure_new_focedPhot(img ,lut1, lut2, guessFlux = 100, guessmux = 0, guessmuy=0 , guessAlphax =3, guessAlphay =3, guessAlphaxy =0 , counter_target = 100, distort = 0, fixBkg=0, back_calc=0):
    #Shape the image properly
    img = np.array(img)
    sizey, sizex = np.shape(img)
