...
#plt.xlabel('Log_{10}(Flux/sqrt(b)*A)')    
#plt.ylabel(r'$ \frac{f_1}{A.\sqrt{B}}$ ')
plt.ylabel(r'$g_1$')
plt.xlabel(r'$Log_{10}(\frac{N}{A.\sqrt{B}})$')    
plt.plot(final[:,0], final[:,2]*2, 'r-', markersize= 10, label = 'Fitted Function')
plt.plot(np.arange(-4,-3.3,0.01), 2*np.ones(71)*final[0,2], 'r-', markersize= 10)
plt.plot(np.arange(2.125,3,0.01), 2*np.ones(88)*final[217,2], 'r-', markersize= 10)
