...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb  7 18:17:24 2023
...
fluxArr =np.array([ 20000, 30000, 50000, 70000])
sigx = 6
sigy = 4
sigxy = 12
bkg = 200
...
for flux in fluxArr:
    ...
    for j in range(200):
        ...
        flux_measure, mux_measure, muy_measure, e1_measure, e2, bkg_measure, psf_measure, sigxx_measure,sigyy_measure, sigxy_measure = measure(tot, sigx, sigy, sigxy, 100)
        if(flux_measure == None or sigxx_measure == None or np.isnan(flux_measure) or np.isnan(psf_measure) or psf_measure == None):
            continue
        
        if(abs(sigxx_measure - sigx**2/2) > 100 or  abs(sigyy_measure - sigy**2/2) > 100):
            continue
        if(sigxx_measure < 0 or sigxx_measure > 100 or sigyy_measure<0 or sigyy_measure>100):
            continue
    ...
    print (fbysqrtba, 'aaaaa', len(tempArr_sigx))
    if(fbysqrtba< 1.175):
        index = 1
    elif(fbysqrtba> 2.875):
        index = 34
    else:
