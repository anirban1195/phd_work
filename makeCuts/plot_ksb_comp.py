...
for j in range(3,75, 2):
    a.append(np.sum(data[360-j:360+j+1, 230-j:230+j+1])*6.28*0.7*5.59e-4)
    b.append(j*50*0.11)
a=np.array(a)    
plt.errorbar(b, a, yerr=0.1*a , fmt='r.', label='This work')
...
# =============================================================================
#     temp=content[12+j].split()
#     ksb_mass_up.append((float(temp[1][0:6]))/1e15/0.7)
#     temp=content[23+j].split()
#     ksb_mass_down.append((float(temp[1][0:6]))/1e15/0.7)
...
plt.errorbar(ksb_radius, ksb_mass,  fmt='.k', label='SKF')
