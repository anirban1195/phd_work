...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 13 09:29:21 2023
...
def EandB(ir_coadd_data_name, r_coadd_npy_name, i_coadd_npy_name, zFile, outFile, coadd_img, r_sf_name, i_sf_name):
    ...
    #for j in [17420]:   
        ...
        if(ir_coadd_data [j,3] < 1.0e10 or r_coadd_npy[j,3]<=0 or i_coadd_npy[j,3]<=0 ):
...
# =============================================================================
    ...
    for j in range(110, 560):
        ...
        for k in range(100, 520):
            ...
            cond = np.where((master_frame[:,0] > x_mid-1000) & (master_frame[:,0] < x_mid+1000) & 
                            (master_frame[:,1] > y_mid-1000) & (master_frame[:,1] < y_mid+1000) 
            ...
            goodEllipIndices = np.where( (r2<(alphax)**2) & (r2>400) & (tot_ellip > 0.0) )[0]
    ...
    hdu.writeto('/scratch/bell/dutta26/abell_2390/local_density_coadd.fits', overwrite=True)
    
   
    
                
img = '/scratch/bell/dutta26/abell_2390/abell_ir_coadd.fits'
ebModeFile = '/scratch/bell/dutta26/abell_2390/master_arr_coadd.npy'
...
f=fits.open('/scratch/bell/dutta26/abell_2390/EMode_coadd1.fits')
e_mode = f[0].data
f.close()

f=fits.open('/scratch/bell/dutta26/abell_2390/BMode_coadd1.fits')
b_mode = f[0].data
f.close()

f=fits.open('/scratch/bell/dutta26/abell_2390/local_density_coadd.fits')
density_mode = f[0].data
f.close()
...
sizey, sizex = np.shape(e_mode)
temp = np.zeros((sizey, sizex))
temp_b = np.zeros((sizey, sizex))

for j in range(0, sizey, 1):
    for k in range(0 ,sizex, 1):
        avg_den = np.median(density_mode[j:j+2, k:k+2])
        fact = 1/(1 + 3/(avg_den**1+1))
        #temp[j:j+3, k:k+3] = np.sum(e_mode[j:j+3, k:k+3]*density_mode[j:j+3, k:k+3]**1.5)/10000
        #temp_b[j:j+3, k:k+3] = np.sum(b_mode[j:j+3, k:k+3]*density_mode[j:j+3, k:k+3]**1.5)/10000
        #temp[j:j+2, k:k+2] = np.sum(e_mode[j:j+2, k:k+2]*fact)
        #temp_b[j:j+2, k:k+2] = np.sum(b_mode[j:j+2, k:k+2]*fact)
        temp[j:j+1, k:k+1] = np.sum(e_mode[j:j+1, k:k+1]*density_mode[j:j+1, k:k+1]**1.5)/10000
        temp_b[j:j+1, k:k+1] = np.sum(b_mode[j:j+1, k:k+1]*density_mode[j:j+1, k:k+1]**1.5)/10000
        
hdu = fits.PrimaryHDU(temp, header=header)
hdu.writeto('/scratch/bell/dutta26/abell_2390/E_rebinned.fits', overwrite=True)


hdu = fits.PrimaryHDU(temp_b, header=header)
hdu.writeto('/scratch/bell/dutta26/abell_2390/B_rebinned.fits', overwrite=True)