...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 26 12:16:50 2021
...
def make(band):
    ...
    indices = np.where((fluxList>50) & (fluxList<1000))[0]
    print (len(indices))
    raList1  = []
    decList1 =[]
    ...
    for j in range(len(xList)):
        ...
        if(flux == None):
            ...
            continue
        plt.plot(np.sqrt(sigxx+sigyy), flux , 'k.', markersize = 1)

        sizeList.append (3*psf)
        sigxxList.append(sigxx)
        sigyyList.append(sigyy)
        sigxyList.append(sigxy)
        raList2.append(raList1[j])
        decList2.append(decList1[j])
    ...
    plt.xlabel("Size")
    return (0)
    store = np.zeros((120, len(raList2), 15), dtype = np.float64)
    counter = 0
    failedArr=[]
...
# =============================================================================
            ...
            xList, yList = helper.convertToXY(raList2, decList2, '/scratch/halstead/d/dutta26/abell_2390/temp_coadd.fits')
    ...
    # =============================================================================
                #print (j)
                if((j not in indexList) or (sizeList[j] == None)):
                    continue
                x = int(round( xList[j]))
                y = int(round(yList[j]))
                if(x<0 or x> sizex):
    ...
    np.save('/home/dutta26/test14_uncorrected_' +str(band)+'.npy', store)
    return failedArr
