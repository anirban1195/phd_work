...
z_sf_df = np.load('/scratch/bell/dutta26/abell_2390/z_sf.npy')
master=[]
seeing =[]
for j in range(len(r_sf_df)):
    ...
    master.append(seeing[len(seeing)-1])
...
for j in range(len(z_sf_df)):
    loc = np.where(z_sf_df[j,:,16] > 0)
    if(len(loc[0]) == 0):
        
        continue
    seeing.append(np.median(z_sf_df[j,loc,16]))
    master.append(seeing[len(seeing)-1])
print (len(seeing))
plt.hist(x=seeing, bins=15, density=True,histtype=u'step', color='y', label='z')
seeing=[]  
