...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  8 08:33:14 2022
...
for j in range(len(xList)):
    ...
    if(flux == None or np.isnan(flux)):
        ...
        if(flux> 2000 and flux< 3000 and store[j,2] == 1 and store[j,13] == 1 and sigxx<4):
            sys.exit()
...
star_arr = store[(np.where((store[:,2] == 1)& (store[:,3] > 10000)& (store[:,3] < 100000) & (store[:,13] == 1) ))[0],  : ]
...
# 
# n, bins, patches = plt.hist(x=star_arr[:,7], bins=50, color='r',
#                                alpha=0.7, rwidth=0.85, density=True)
# 
# for files in os.listdir('/scratch/halstead/d/dutta26/abell_2390/obj1/'):
#     os.remove('/scratch/halstead/d/dutta26/abell_2390/obj1/'+files)
# for j in loc:
#     if(store[j,7]> 14):
#         continue
#     x = int(round( xList[j]))
...
for j in range(len(xList)):
    x=int(xList[j])
    y=int(yList[j])
    if(store[j,2] == 1  or min_dist_arr[j]< 20 or store[j,3] < 300) :     
        continue
...
np.save('/scratch/halstead/d/dutta26/abell_2390/wiyn_data_bkg.npy', store)
