...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep  6 17:27:32 2023
...
ir_coadd_df = np.load('/home/dutta26/codes/wiyn_wl_sim/coaddSc_ir.npy')
#Read the npy file
#r_sf_df = np.load('/scratch/bell/dutta26/abell_2390/test2_r.npy')
r_sf_df = np.load('/scratch/bell/dutta26/wiyn_sim/i.npy')
pArr =[]
sArr =[]
kArr =[]
eArr= []
...
fArr=[]
airArr=[]
for slice_no in np.arange(0,30, 1):
#for slice_no in [32]:
    #slice_no = 8
    ...
    maxFlux = 10**5 * (scale_def/scale)
    minFlux = 10**4 * (scale_def/scale)
    loc = np.where((r_sf_df[slice_no,:,3]>minFlux) & (r_sf_df[slice_no,:,3]< maxFlux) &
                  ...
                  (r_sf_df[slice_no,:,7]> mean-6*std) & (r_sf_df[slice_no,:,8]> mean1-6*std1) & 
                  (r_sf_df[slice_no,:,2] == 1)& (ir_coadd_df[:,79] == 0))[0]
    
    if(len(loc)< 100):
        #print ('bb')
        continue
    data = r_sf_df[slice_no,loc,7]
    ...
    airArr.append(np.median( star_arr[:,19]))
    ...
    err_xx = err_yy = np.sqrt( (mean_psf_size**4/fluxArr + (4* np.pi* mean_psf_size**6 * bkg)/(fluxArr**2) ) )
    err_xy = err_xx * 0.707
    ...
    yVal =[]
    tot = 0
    start = mean_xx - 10*np.sqrt(k_guess_xx**2 + err_xx**2)
    cutoff = mean_xx + 10*np.sqrt(k_guess_xx**2 + err_xx**2)
    #center = 13.6
    ...
    #print (parameters)
    kArr.append(np.sqrt(parameters[1]**2 - err_xx**2))
...
bArr =np.array(bArr)
fArr=np.array(fArr)
#plt.plot(np.arange(1,8, 0.1), np.arange(1,8, 0.1)**4/900, 'b-', label = r'$y = x^4/900$')
plt.plot(pArr, kArr**2, 'k.')
# =============================================================================
# plt.scatter(pArr, kArr**2, c=airArr, cmap='viridis')
# plt.colorbar()
# =============================================================================
plt.xlabel('Mean PSF Size')
plt.ylabel(r'Systematic Error in xx $^2$')
plt.legend()
# =============================================================================
