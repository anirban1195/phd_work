...
ref_catalog = '/scratch/bell/dutta26/wiyn_sim/wcs_reference.cat'
...
def correct_wcs(idNo, filt, loc_img):
    ...
    #bashCommand = 'sex '+ file +' -DETECT_MINAREA 9 -DETECT_THRESH 5 -ANALYSIS_THRESH 5 -WEIGHT_TYPE MAP_WEIGHT -WEIGHT_IMAGE '+loc_img+filt+'/'+str(idNo)+'/output.weight.fits'+' -CHECKIMAGE_TYPE NONE -CATALOG_NAME /scratch/bell/dutta26/wiyn_sim/temp.cat'
    bashCommand = 'sex '+ file +' -DETECT_MINAREA 5 -DETECT_THRESH 1.5 -ANALYSIS_THRESH 1.5 -WEIGHT_TYPE MAP_WEIGHT -WEIGHT_IMAGE '+loc_img+filt+'/'+str(idNo)+'/output.weight.fits'+' -CHECKIMAGE_TYPE NONE -CATALOG_NAME /scratch/bell/dutta26/wiyn_sim/temp.cat'

    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE, cwd=sexLoc)
    output, error = process.communicate()
    
    #Read the seatractor output
    f=open('/scratch/bell/dutta26/wiyn_sim/temp.cat')
    cat = f.readlines()
    f.close()
    raArr =[]
    decArr=[]
