...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov  5 08:45:50 2023
...
def get_avg_obs(datafile, img_file, cent_ra, cent_dec, radius, zLens, ir_coadd_file):
    ...
    loc = np.where( (distArr <= radius) & (master_frame[:,4,0]> 1) & (ir_coadd_data[:,2]==0) & (ir_coadd_data[:,3]>10)
    ...
    th_arr =[]
    z_arr=[]
    thresholding_err_arr = []
    signalArr =[]
    #print (ir_coadd_data[0,1],ir_coadd_data[0,1])
    for ind in loc:
        loc1 = np.where(master_frame[ind, 1,:]>0)[0]
        for j in loc1:
            
            thresholding_err_arr.append(master_frame[ind, 15, j])
            
    
    med_err = np.median(thresholding_err_arr)
    
    print (len(loc), '***************', med_err)
    reject = 0
    for ind in loc:
        loc1 = np.where(master_frame[ind, 1,:]>0)[0]   
        #print (len(loc), len(loc1))
        ...
        for j in loc1:
            e1 = master_frame[ind,2,j]
            e2 = master_frame[ind,3,j]
            #print (e1,e2)
            size = np.sqrt(master_frame[ind,6,j] + master_frame[ind,7,j])
            #print (size, ind, master_frame[ind, 15,j])
            ...
            ellip_err = master_frame[ind, 15,j]
            ...
            if(wt>1/(med_err/3)**2):
                wt=1/(med_err/3)**2
            #print (wt)
            if(abs(e1)>5 or abs(e2)>5 or tot_ellip==0 or np.isnan(size) or size<0.15 or size>10 ):
                wt = 0
                
            
            epar= - (e1*cos2phi + e2*sin2phi)
            if(abs(e1)>5 or abs(e2)>5 or tot_ellip==0 or np.isnan(size) or size<0.15 or size>10 or wt<=0 ):
                reject += 1
                continue
            else:
                ...
                cnt += (1*wt)
                wtArr.append(wt)
                errArr.append(master_frame[ind, 15,j])
    ...
    print (len(np.where(ellipArr>1)[0]), np.max(ellipArr), len(np.where(ellipArr<=0.01)[0]), reject)
    ...
    print (len(ellipArr), np.median(ellipArr**2), cnt, totShear, np.sum(ellipArr**2 * wtArr**0.5)/np.sum(wtArr**0.5), np.sum(ellipArr**2 *wtArr)) 
...
finalDataSet_loc='/scratch/bell/dutta26/wiyn_sim/master_arr_coaddMC_sfMC_all.npy'
...
bins = np.arange(50/3600, 600/3600, 30/3600 ) 
th_avg_shear = []
obs_avg_shear =[]
obs_shear_err=[]
for radius in bins:
...
plt.errorbar(bins*3600, obs_avg_shear,  yerr= obs_shear_err, fmt='k.', label = 'Giving Shear code everything',capsize=3)
...
#     #th_avg_shear.append(get_avg_th(328.3941 , 17.6697, radius, 0.3))
#     th_avg_shear.append(-0.1)
#     shear_obs, error, th_med = get_avg_obs(finalDataSet_loc, imgLoc, [328.3944], [17.6695], radius, 0.3, ir_coadd_file )
#     obs_avg_shear.append(shear_obs)
#     obs_shear_err.append(error)
#     #break
#     #th_avg_shear.append(-th_med)
