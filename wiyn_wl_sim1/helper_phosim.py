...
def clean(folder, catalog):
    ...
    f.close()    
    swarpCommand = './swarp @'+folder+'temp.txt'+' -c /home/dutta26/codes/wiyn_wl_sim1/default.swarp -IMAGEOUT_NAME '+folder+'output.fits -WEIGHTOUT_NAME '+folder+'output.weight.fits -SUBTRACT_BACK N  -WEIGHT_TYPE NONE '
...
def make_phosim_imputCatalog(idNo, filt):
    ...
    f=open('/scratch/bell/dutta26/wiyn_sim1/sheared_uniform.cat')
    content = f.readlines()
    f.close()
    ...
    f=open('/scratch/bell/dutta26/wiyn_sim1/'+str(uid)+'.txt', 'w+')
    ...
    for j in range(len(content)):
        f.write(content[j])
    f.close()
    return '/scratch/bell/dutta26/wiyn_sim1/'+str(uid)+'.txt'
...
def make_phosim_star_imputCatalog(idNo, filt):
    ...
    count = 1
    uid = filt+'_'+idNo
    f=open('/scratch/bell/dutta26/wiyn_sim1/star_'+str(uid)+'.txt', 'w+')
    f.write('rightascension '+str(ra)+'  \n')    
    f.write('declination '+str(dec)+'  \n')
    f.write('seed '+str(idNo)+ '\n')
    f.write('obshistid '+str(idNo)+ '\n')
    ...
    for j in range(len(raArr)):
        ...
        for k in range(len(decArr)):
            ...
            count += 1
    f.close()
    return '/scratch/bell/dutta26/wiyn_sim1/star_'+str(uid)+'.txt'


def makeWeight(idNo, filt, loc_img):
    ref_catalog_star = '/scratch/bell/dutta26/wiyn_sim1/wcs_reference_star.cat'
